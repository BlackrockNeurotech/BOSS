...
void GLWidget::drawPoints()
{
	...
	if ((featuresData) && (unitData))
	{
		...
		glBegin(GL_POINTS);
		{
			...
			{
                double vX = featuresData->getFeatureValueForIndex(selectionIndexes[i], featureX) * 100;
                double vY = featuresData->getFeatureValueForIndex(selectionIndexes[i], featureY) * 100;
                double vZ = featuresData->getFeatureValueForIndex(selectionIndexes[i], featurez) * 100;
        ...
        for (int i = 0; i < pClusterData->nbrClusters(); i++)
        {
            ...
            double vX = pClusterData->getFeatureValueNormalized(featuresData, i, getFeatureID(0)) * 100;
            double vY = pClusterData->getFeatureValueNormalized(featuresData, i, getFeatureID(1)) * 100;
            double vZ = pClusterData->getFeatureValueNormalized(featuresData, i, getFeatureID(2)) * 100;
...
bool GLWidget::processSingleSelection(int x, int y, bool clear)
{
	...
	if (featuresData)
	{
		...
		{
			ccm->setData(ccm->index(channel, CC_NBR_SORT_UNITS), QVariant(centroids.size()), Qt::DisplayRole);
...
void GLWidget::renderSelection(int size)
{
    ...
    if (featuresData)
    {
		...
		glBegin(GL_POINTS);
		{
			for (int i = 0; i < wfIDs.size(); i++)
			{
				if (!m_selected.contains(i))// && !m_centroids.contains(i))
				{
